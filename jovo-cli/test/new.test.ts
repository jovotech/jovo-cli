...
describe('new', () => {
  it('jovo new <project>', async () => {
    ...
    expect(JSON.parse(readFileSync(path.join(projectFolder, 'package.json'), 'utf-8')).name).toBe(
      'jovo-sample-voice-app-nodejs',
    );
    expect(existsSync(path.join(projectFolder, 'src', 'app.js'))).toBe(true);
    expect(existsSync(path.join(projectFolder, 'src', 'index.js'))).toBe(true);
    expect(existsSync(path.join(projectFolder, 'src', 'config.js'))).toBe(true);
  ...
  it('jovo new helloworld --locale de-DE', async () => {
    ...
    // Tests
    expect(existsSync(path.join(projectFolder, 'models', 'de-DE.json'))).toBe(true);
  }, 10000);
});
