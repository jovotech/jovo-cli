...
import { ListrRenderer, ListrOptions } from 'listr';
...
const intdentText = (
	...
	do {
		...
		if (
			textLeft &&
			tempText.charAt(maxTextLength - 1) !== ' ' &&
			textLeft.charAt(0) !== ' '
		) {
			...
			if (lastSpacePosition !== -1) {
				textLeft = (
					tempText.substring(lastSpacePosition) + textLeft
				).trim();
		...
		if (firstLinePrefix) {
			if (itteration === 0) {
				...
				fullIdentText =
					fullIdentText + ' '.repeat(firstLinePrefix.length);
			}
		}
		// @ts-ignore
...
const renderHelper = (
	...
	for (const task of tasks) {
		if (task.isEnabled()) {
			...
			output.push(
				indentString(
					` ${getSymbol(task, options)} ${task.title}`,
					level,
					{ indent: '  ' }
				)
			);
				...
				if (isDefined(data)) {
					if (data.substr(0, 5) === 'Info:') {
						...
						for (const item of arr) {
							output.push.apply(
								output,
								intdentText(item, level, '  ', 'red', ' ')
							);
						}
					} else {
						output.push.apply(
							output,
							intdentText(data, level, '  ', 'grey', ' -> ')
						);
					}
				}
			}
			...
			if (
				...
				output = output.concat(
					renderHelper(task.subtasks, options, level + 1)
				);
			}
		}
	}
