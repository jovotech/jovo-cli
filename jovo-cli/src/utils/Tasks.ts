	...
	for (const locale of locales) {
		validationTasks.push({
			...
			async task(ctx: JovoTaskContext, task: ListrTaskWrapper) {
				try {
					modelFileContent = await project.getModelFileJsonContent(
						locale
					);
				} catch (err) {
					if (err.code === 'ENOENT') {
						...
						if (existsSync(project.getModelPath(locale, 'js'))) {
							return task.skip(
								'Model file is of type .js, not .json, so check got skipped.'
							);
						}
				...
				try {
					...
					throw new Error(
						`Model-File is not valid JSON: ${err.message}.`
					);
				}
				...
				if (
					ctx.ignoreTasks &&
					ctx.ignoreTasks.includes('model-validation')
				) {
					return;
				}
...
export function deployTask(ctx: JovoTaskContext): ListrTask[] {
	...
	if (
		ctx.targets.includes(TARGET_ZIP) &&
		!preDeployTasks.includes(TARGET_ZIP)
	) {
