...
export async function getPlatformFiles(): Promise<NativeFileInformation[]> {
  ...
  const foldersIncludeMain = ['entities', 'intents'];
  ...
  for (const subfolder of foldersIncludeMain) {
    ...
    for (fileName of subfolderFiles) {
      platformFiles.push({
        ...
        content: JSON.parse(
          await readFile(pathJoin(platformFolderPath, subfolder, fileName), 'utf8'),
        ),
      });
    }
  }
...
export function buildDialogFlowAgent(ctx: JovoTaskContextGoogle) {
  return new Promise((resolve, reject) => {
    try {
      ...
      let url = null;
      url =
        _.get(config, 'googleAction.dialogflow.endpoint') ||
      ...
      if (ctx.locales && ctx.locales.length === 1) {
        ...
        primaryLanguage =
          _.get(config, 'googleAction.dialogflow.primaryLanguage') || primaryLanguage;
...
export const v2 = {
  ...
  activateServiceAccount(config: JovoTaskContextGoogle): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        exec(
          'gcloud auth activate-service-account --key-file="' + config.keyFile + '"',
