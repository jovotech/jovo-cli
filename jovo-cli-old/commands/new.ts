			...
			try {
				...
				if (!args.directory) {
					...
					if (!isValidProjectName(args.directory)) {
						console.log(
							`The folder name "${args.directory}" is not valid`
						);
						return;
					}
				}
				...
				if (
					args.directory &&
					project.hasExistingProject(args.directory)
				) {
				...
				if (args.options.init) {
					if (project.frameworkVersion !== 1) {
						console.error(
							'The "init" option got deprecated and is only available for Jovo Framework v1 projects.'
						);
						return;
					}
				}
				...
				for (let i = 0; i < platformFlags.length; i++) {
					if (args.options.hasOwnProperty(platformFlags[i])) {
						if (
							typeof args.options[platformFlags[i]] === 'boolean'
						) {
				...
				if (args.options.deploy) {
					...
					if (!args.options.init) {
						console.log(
							'Please use --init <platform> if you use --build'
						);
						return;
					}
				}
				...
				tasks.add({
					...
					task: ctx => {
						return project
							...
							.then(() => {
								return project.updateModelLocale(
									ctx.locales[0]
								);
							});
					}
