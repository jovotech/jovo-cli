...
import { getProject, Utils, JOVO_WEBHOOK_URL } from 'jovo-cli-core';
...
module.exports = (vorpal: Vorpal) => {
	...
	const vorpalCommand = vorpal
		...
		.option(
			...
			'Disables Jovo Debugger (web version)'
		)
	...
	vorpalCommand.action(async (args: Args) => {
		...
		if (
			project.jovoConfigReader!.getConfigParameter('src', stage) as
				| string
				| undefined
		) {
			srcDir = project.jovoConfigReader!.getConfigParameter(
				'src',
				stage
			) as string | undefined;
		...
		const localServerFile =
			args.webhookFile === 'index.js' ? 'index.js' : args.webhookFile;
		let command = 'node';
		...
		if (args.options['bst-proxy']) {
			...
			proxy.start(() => {
				const data = fs.readFileSync(
					path.join(Utils.getUserHome(), '.bst/config')
				);
				const bstConfig = JSON.parse(data.toString());

				const proxyURL =
					'https://' + bstConfig.sourceID + '.bespoken.link/webhook';
				const dashboardURL =
					'https://bespoken.tools/dashboard?id=' +
					bstConfig.sourceID +
					'&key=' +
					bstConfig.secretKey;
				let messageOutput =
